...
static unsigned int sqlctx = 150611;
...
code            ";
 ...
 static char *sq0008 = 
"select empno ,paybank ,paybank1 ,payacnt ,payacnt1 ,realpay ,(nvl(mcaramt,0\
)-nvl(ticketamt,0)) mcamt ,(nvl(realpay,0)-(nvl(mcaramt,0)-nvl(ticketamt,0)))\
 mc_real  from pkmpcalc where (empno not like 'Y%' and (paybank=:b0 or payban\
k1=:b0)) union all select empno ,paybank ,nvl(trim(paybank1),paybank) paybank\
1 ,payacnt ,nvl(trim(payacnt1),payacnt) payacnt1 ,nvl(realpay,0) realpay ,(((\
((nvl(taxpay1,0)+nvl(taxpay2,0))+nvl(taxpay3,0))+nvl(taxpay4,0))+nvl(taxpay5,\
0))+nvl(notaxpay4,0)) mcamt ,(nvl(realpay,0)-(((((nvl(taxpay1,0)+nvl(taxpay2,\
0))+nvl(taxpay3,0))+nvl(taxpay4,0))+nvl(taxpay5,0))+nvl(notaxpay4,0))) mc_rea\
l  from pkzhphis where (((payyn='Y' and nvl(realpay,0)>0) and paydate=(select\
 deddate  from pkclient where bacode=:b2)) and (Nvl(paybank,'ZZ')=:b0 or Nvl(\
paybank1,'ZZ')=:b0)) order by empno            ";
...
{12,4274,840,0,0,
5,0,0,1,82,0,4,136,0,0,4,1,0,1,0,2,97,0,0,2,97,0,0,2,97,0,0,1,97,0,0,
36,0,0,2,63,0,4,173,0,0,1,0,0,1,0,2,97,0,0,
55,0,0,3,46,0,4,200,0,0,1,0,0,1,0,2,3,0,0,
74,0,0,4,184,0,3,216,0,0,13,13,0,1,0,1,3,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,
0,1,3,0,0,1,3,0,0,1,4,0,0,1,4,0,0,1,3,0,0,1,4,0,0,1,3,0,0,1,4,0,0,
141,0,0,5,0,0,29,230,0,0,0,0,0,1,0,
156,0,0,6,161,0,4,261,0,0,10,1,0,1,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,
0,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,0,0,1,97,0,0,
211,0,0,7,91,0,9,330,0,0,2,2,0,1,0,1,97,0,0,1,97,0,0,
234,0,0,7,0,0,13,343,0,0,1,0,0,1,0,2,97,0,0,
253,0,0,7,0,0,15,348,0,0,0,0,0,1,0,
268,0,0,7,0,0,15,355,0,0,0,0,0,1,0,
283,0,0,8,815,0,9,405,0,0,5,5,0,1,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,
0,
318,0,0,8,0,0,13,419,0,0,8,0,0,1,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,0,
0,2,3,0,0,2,3,0,0,2,3,0,0,
365,0,0,8,0,0,15,427,0,0,0,0,0,1,0,
380,0,0,8,0,0,15,434,0,0,0,0,0,1,0,
395,0,0,9,50,0,3,709,0,0,5,5,0,2,6,76,79,71,95,68,66,1,97,0,0,1,97,0,0,1,3,0,0,
1,97,0,0,1,97,0,0,
436,0,0,10,0,0,29,719,0,0,0,0,0,2,6,76,79,71,95,68,66,
};
...
SetBody()
{
     ...
     where   empno not like 'Y%'              /oY사번 제외 Add  dsa2000  2008.01.o/
       /oand   empno < '0260'                   jissi 임시 테스트자료 100건이하 추출o/
     ...
     SELECT  empno, paybank, nvl(trim(paybank1),paybank) paybank1, 
             ...
             o/
             /o 2016.02.23 jissi HR팀 김보배 요청 분리계좌에 급여항목 삭제(기타과세6) 및 추가(기타비과세4)
...

{
     ...
     sqlstm.sqadto[1] = (unsigned short )0;
     sqlstm.sqtdso[1] = (unsigned short )0;
     sqlstm.sqhstv[2] = (unsigned char  *)bacode;

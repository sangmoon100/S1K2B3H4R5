...
static unsigned int sqlctx = 150851;
...
code            ";
 ...
 static char *sq0007 = 
"select tcompany ,p.paybank ,p.paycnt ,sum(realpay) realpay  from pkmpycalc \
c ,pimpmas m ,pkmyemcorp p where ((m.tcompany=p.corpcode and c.empno=m.empno)\
 and p.paybank=:b0) group by tcompany,p.paybank,p.paycnt           ";
...
{12,4274,840,0,0,
5,0,0,1,82,0,4,131,0,0,4,1,0,1,0,2,97,0,0,2,97,0,0,2,97,0,0,1,97,0,0,
36,0,0,2,46,0,4,176,0,0,1,0,0,1,0,2,3,0,0,
55,0,0,3,184,0,3,192,0,0,13,13,0,1,0,1,3,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,
0,1,3,0,0,1,3,0,0,1,4,0,0,1,4,0,0,1,3,0,0,1,4,0,0,1,3,0,0,1,4,0,0,
122,0,0,4,0,0,29,206,0,0,0,0,0,1,0,
137,0,0,5,161,0,4,237,0,0,10,1,0,1,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,
0,0,2,97,0,0,2,97,0,0,2,97,0,0,2,97,0,0,1,97,0,0,
192,0,0,6,91,0,9,323,0,0,2,2,0,1,0,1,97,0,0,1,97,0,0,
215,0,0,6,0,0,13,336,0,0,1,0,0,1,0,2,97,0,0,
234,0,0,6,0,0,15,341,0,0,0,0,0,1,0,
249,0,0,6,0,0,15,348,0,0,0,0,0,1,0,
264,0,0,7,219,0,9,372,0,0,1,1,0,1,0,1,97,0,0,
283,0,0,7,0,0,13,386,0,0,4,0,0,1,0,2,97,0,0,2,97,0,0,2,97,0,0,2,3,0,0,
314,0,0,7,0,0,15,397,0,0,0,0,0,1,0,
329,0,0,7,0,0,15,404,0,0,0,0,0,1,0,
344,0,0,8,50,0,3,630,0,0,5,5,0,2,6,76,79,71,95,68,66,1,97,0,0,1,97,0,0,1,3,0,0,
1,97,0,0,1,97,0,0,
385,0,0,9,0,0,29,640,0,0,0,0,0,2,6,76,79,71,95,68,66,
};


/* ======================= Program Header ======================================
 PROGRAM-NAME   : PKC6050A(파견직 급여(업체) 이체  파일생성)
 SYSTEM-NAME    : 급여
...
{        
     ...
     if  (argc != 8) {  /* /hper8/HINSA/proc/bin/Kbin/pkc6050a 05 0404051110.lst 20040402 20100331 D006 pkc6050g 2004110100000 */
          printf("[Usage] : pkc60500a 1.은행코드 2.파일명      3.지급일자 4.파견직급여기산일    \n");
     ...
     /* Dsa2000  2004.02.24.  **********************************/  
     /*DB_connect(id,0); */  
     hinsa_log_print(0,"파견직 금액이체 파일생성 프로그램 시작...[pkc6050a]");
...
/*===========================================================================================*/  
     ...
     {
          sprintf(log_buff, "OK ====== [파견직 급여  계산 작업성공] =====\n");
          Write_batlog(seqno++, log_buff);   /*dsa2000 Rexec 대체*/
          hinsa_exit(0,"OK ====== [파견직 급여  계산 작업성공] =====\n");    
     }
...
SetBody()
{
     ...
     select tcompany,p.paybank,p.paycnt,sum(realpay) realpay
       from pkmpycalc c,pimpmas m,pkmyemcorp p
      where m.tcompany = p.corpcode 
        and c.empno    = m.empno
        and p.paybank  = :bankcode
   group by tcompany,p.paybank,p.paycnt; */ 
...

{
     ...
     if  (sqlca.sqlcode != 0 && sqlca.sqlcode != 1403 )
     {
          printf("[ERRCODE : %d] 이체금액111111 setting 에러\n",sqlca.sqlcode);
          fclose(fp);
          unlink(FullName);
          sprintf(log_buff, "[ERRCODE : %d] 이체금액22222 setting 에러\n",sqlca.sqlcode);
          Write_batlog(seqno++, log_buff);   /*dsa2000 Rexec 대체*/
          exit(1);
     }
...

{
          ...
          /* **********
          printf("leerk tcompany %s sqlcode =>%d,%s,%s 읽기 에러\n",tcompany,sqlca.sqlcode,caldate,paybank);     
          printf("leerk tcompany %s sqlcode =>%d 읽기 에러\n",tcompany,sqlca.sqlcode);
          * ********** */
          if  (sqlca.sqlcode == 1403)
          {
               /* EXEC SQL CLOSE c1; */ 

...
{12,4274,840,0,0,
5,0,0,1,0,0,24,115,0,0,1,1,0,1,0,1,97,0,0,
24,0,0,2,0,0,24,139,0,0,1,1,0,1,0,1,97,0,0,
43,0,0,3,0,0,24,151,0,0,1,1,0,1,0,1,97,0,0,
62,0,0,4,50,0,3,162,0,0,5,5,0,2,6,76,79,71,95,68,66,1,97,0,0,1,97,0,0,1,3,0,0,
1,97,0,0,1,97,0,0,
103,0,0,5,0,0,29,174,0,0,0,0,0,2,6,76,79,71,95,68,66,
};


/* ======================= Program Header ======================================
 PROGRAM-NAME   : pkc1030g.pc  (maketemp)(월 변동자료 추출)
 SYSTEM-NAME    : 급여
 SUBSYSTEM-NAME : 급여계산
 Programmer     : 이인환
 Version        : 1.00
...
Update Contents
   ...
   35.00     2004.05.21          강륜종      파일명 maketemp.pc 에서 pkc1030g.pc 로 변경.
   36.00     2004.11.       강륜종(dsa2000)  Rexec대체 서비스를 위한 수정작업.      
   37.00     2015.04.            강륜종      변동테이블을 고정테이블로 변경.
============================================================================= */
...
#include "hinsa_common.h" /*dsa2000  추가.. 2004.11.03.*/
...
char *argv[];
{
     ...
     if  (argc != 6) {  /* pkc1030g /hper/insa/HINSA/data/Kdata/T_SPTMAMT SPTMAMT D006 pkc1030g 2004110100000 */
          printf("[Usage] : pkc1030g 1.경로+Tname 2.datafld 3.writeman 4.progid 5.rundate \n");
          exit(1);
     }

     /*로그 디렉토리 생성 및 로그작업*/
     STRINIT(FL_file);
     strcpy(FL_file,"pkc1030g");
...
char *datafld;
{
     ...
     /*2015.04. 계정권한 분리작업 : Delete구문 Add */
     sprintf(sqlstr,"Delete PKC1030G_Program Where Workemp='%s' And Datafld='%s'", log_writeman,datafld);
     /* EXEC SQL EXECUTE IMMEDIATE :sqlstr; */ 

{
     ...
     while (fgets(buf,256,fp))
     {
          ...
          if  (strncmp(datafld, "NOJOYN",6) == 0 ) /*dsa2000  2005.10 문자값 처리토록 추가.*/
          {
               ...
               sprintf(sqlstr,"insert into PKC1030G_Program (Workemp,Datafld,Empno,Charvalue) Values ('%s','%s','%s','%s')"
          ...
          else
          {
               ...
               sprintf(sqlstr,"insert into PKC1030G_Program (Workemp,Datafld,Empno,Numvalue) Values ('%s','%s','%s',%s)"

...
static unsigned int sqlctx = 308755;
...
51,0,0,3,784,0,3,271,0,0,13,13,0,1,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,3,0,
0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,1,97,0,0,
118,0,0,4,147,0,4,346,0,0,5,3,0,1,0,2,4,0,0,2,4,0,0,1,97,0,0,1,97,0,0,1,3,0,0,
153,0,0,5,50,0,3,361,0,0,5,5,0,2,6,76,79,71,95,68,66,1,97,0,0,1,97,0,0,1,3,0,0,
1,97,0,0,1,97,0,0,
194,0,0,6,0,0,29,371,0,0,0,0,0,2,6,76,79,71,95,68,66,
};
...
AG,EAI_DATE)select :b0 ,SUBSTR(:b1,1,4) ,SUBSTR(:b1,5,2) ,'GL' MODULE_GU ,:b3\
 ,:b4 ,rownum ,'HR' IFSYS ,:b5 ,((:b3||substr(:b7,1,6))||decode(substr(:b7,7,\
2),'10',decode(substr(:b7,5,2),'07','체단비','11','체단비','명절연봉'),'급여'\
)) ,TO_DATE(:b7,'YYYYMMDD') ,:b11 ,accno ,fincode ,deptcode ,:b12 ,decode(deb\
ityn,'0',nvl(accnosum,0),0) debit ,decode(debityn,'1',nvl(accnosum,0),0) cred\
it ,'I' MODE_FLAG ,'N' EAI_FLAG ,null  EAI_DATE  from V_ERP_ACCNO A ,(select \
ACCNO ACCNOB ,max(DEBITYN) DEBITYN  from ERP_ACCNO  group by ACCNO) where (A.\
ACCNO=ACCNOB and NVL(ACCNOSUM,0)<>0)";
     sqlstm.iters = (unsigned int  )1;
     ...
     sqlstm.sqhstv[9] = (unsigned char  *)paydate;
     ...
     sqlstm.sqhstv[10] = (unsigned char  *)paydate;
     ...
     sqlstm.sqhstv[11] = (unsigned char  *)v_currency;
     ...
     sqlstm.sqhstv[12] = (unsigned char  *)v_activity;
